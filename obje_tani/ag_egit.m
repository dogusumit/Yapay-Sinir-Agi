...
sonuc(i,:)=ag_sina(girisler(i,:),W,B,adet,'yazdirma'); % verilen girisleri ve agirliklari kullanarak sonuc uretiyor
end

subplot(2,1,2);
h1=plot(cikislar(3,:),'g');
hold on;
h2=plot(sonuc(3,:),'k:');
xlabel('Ýndis','FontSize',12,'FontWeight','bold');
ylabel('Deðer','FontSize',12,'FontWeight','bold');
ylim([-0.5 max(cikislar(:))+0.5]);
